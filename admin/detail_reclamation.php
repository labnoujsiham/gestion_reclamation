...
// For demo purposes - in production, get from session
$current_gestionnaire_id = 3; // Gestionnaire Test user
$current_gestionnaire_nom = 'Gestionnaire Test';
    ...
    try {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'request_info') {
            ...
            if ($info_statut) {
                ...
                // Create notification for reclamant
                ...
                if ($recl) {
                    $stmt = $pdo->prepare("
                        INSERT INTO notifications (user_id, type, reference_table, reference_id, contenu, date_creation)
                        VALUES (:user_id, 'demande_info', 'reclamations', :ref_id, :contenu, NOW())
                    ");
                    $stmt->execute([
                        ':user_id' => $recl['user_id'],
                        ':ref_id' => $reclamation_id,
                        ':contenu' => 'Le gestionnaire demande plus d\'informations sur votre réclamation.'
                    ]);
                }
